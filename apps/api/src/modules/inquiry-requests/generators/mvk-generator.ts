/**
 * Генератор справки МВК ЗСО (Межведомственная комиссия по ЗСО)
...
export interface MvkGeneratorParams {
  date: string;
  numberMiddle: string;
  year: string;
  objectName: string;
...
export interface MvkGeneratorResult {
  fileName: string;
  filePath: string;
  buffer: Buffer;
}

export async function generateMvkInquiry(
  templatePath: string,
  outputDir: string,
  params: MvkGeneratorParams,
): Promise<MvkGeneratorResult> {
  ...
  const safeObjectName = params.objectName
    ...
    .substring(0, 30)
    .trim();
  const fileName = `МВК_ЗСО_${safeObjectName}_${Date.now()}.docx`;
