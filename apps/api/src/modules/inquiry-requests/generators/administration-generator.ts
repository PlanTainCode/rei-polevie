/**
 * Генератор справки Администрация муниципального образования (МО)
 * Шаблон: 5. Адм-я.docx
 */
...
export interface AdministrationGeneratorParams {
  date: string;
  numberMiddle: string;
  year: string;
  objectName: string;
  ...
  administrationName?: string; // Название администрации (получатель)
  executors: Executor[];
}

export interface AdministrationGeneratorResult {
  fileName: string;
  filePath: string;
  buffer: Buffer;
}
...
/**
 * Генерирует справку Администрация
 */
export async function generateAdministrationInquiry(
  templatePath: string,
  outputDir: string,
  params: AdministrationGeneratorParams,
): Promise<AdministrationGeneratorResult> {
  ...
  // Заменяем название администрации (получателя)
  if (params.administrationName) {
    docXml = replaceAdministration(docXml, params.administrationName);
  }
  ...
  const safeObjectName = params.objectName
    ...
    .substring(0, 30)
    .trim();
  const fileName = `Администрация_${safeObjectName}_${Date.now()}.docx`;
...
function replaceAdministration(xml: string, newAdmin: string): string {
  // Название администрации в шаблоне разбито на теги, но мы можем заменить ключевые части:
  // 1. "уровское" → остаток нового названия (без "Администрация ")
  // 2. ", Орехово-Зуевского г.о., " → пустая строка
  // 3. "Московской обл." → пустая строка
  // 4. "г", ".", "К" → пустые строки
  
  // Извлекаем часть после "Администрация " из нового названия
  const adminWithoutPrefix = newAdmin.replace(/^Администрация\s*/i, '');
  
  // Заменяем "уровское" на новое название (остаток после "Администрация ")
  xml = xml.replace(/>уровское</g, `>${escapeXml(adminWithoutPrefix)}<`);
  
  // Удаляем остальные части старого названия
  xml = xml.replace(/>, Орехово-Зуевского г\.о\., </g, '><');
  xml = xml.replace(/>Московской обл\.</g, '><');
  
  // Удаляем "г", ".", "К" которые шли перед "уровское"
  // Ищем паттерн: >г<...>.<...>К< и заменяем на пустые теги
  xml = xml.replace(/>г<\/w:t>/g, '></w:t>');
  xml = xml.replace(/>К<\/w:t>/g, '></w:t>');
  // Точка между г и К
