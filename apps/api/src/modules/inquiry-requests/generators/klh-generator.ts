/**
 * Генератор справки КЛХ (Комитет лесного хозяйства МО)
 * Шаблон: 6. КЛХ.docx
 */
...
export interface KlhGeneratorParams {
  date: string;
  numberMiddle: string;
  year: string;
  objectName: string;
...
export interface KlhGeneratorResult {
  fileName: string;
  filePath: string;
  buffer: Buffer;
}

export async function generateKlhInquiry(
  templatePath: string,
  outputDir: string,
  params: KlhGeneratorParams,
): Promise<KlhGeneratorResult> {
  ...
  const safeObjectName = params.objectName
    ...
    .substring(0, 30)
    .trim();
  const fileName = `КЛХ_${safeObjectName}_${Date.now()}.docx`;
