/**
 * Генератор справки Департамент ЖКХ
 * Шаблон: 6. Департамент ЖКХ.docx
 */
...
export interface ZhkhGeneratorParams {
  date: string;
  numberMiddle: string;
  year: string;
  objectName: string;
...
export interface ZhkhGeneratorResult {
  fileName: string;
  filePath: string;
  buffer: Buffer;
}

export async function generateZhkhInquiry(
  templatePath: string,
  outputDir: string,
  params: ZhkhGeneratorParams,
): Promise<ZhkhGeneratorResult> {
  ...
  const safeObjectName = params.objectName
    ...
    .substring(0, 30)
    .trim();
  const fileName = `ДепЖКХ_${safeObjectName}_${Date.now()}.docx`;
