/**
 * Генератор справки Мин Экологии ЗСО (Московская область)
 * Шаблон: 8. Мин Экологии ЗСО.docx
 */
...
export interface MinEcologyGeneratorParams {
  date: string;
  numberMiddle: string;
  year: string;
  objectName: string;
...
export interface MinEcologyGeneratorResult {
  fileName: string;
  filePath: string;
  buffer: Buffer;
}

export async function generateMinEcologyInquiry(
  templatePath: string,
  outputDir: string,
  params: MinEcologyGeneratorParams,
): Promise<MinEcologyGeneratorResult> {
  ...
  const safeObjectName = params.objectName
    ...
    .substring(0, 30)
    .trim();
  const fileName = `МинЭкологии_${safeObjectName}_${Date.now()}.docx`;
...
function replaceDate(xml: string, newDate: string): string {
  ...
  if (parts.length === 3) {
    ...
    const year = `.${parts[2]}`;
    xml = xml.replace(/>29\.08</g, `>${escapeXml(dayMonth)}<`);
    xml = xml.replace(/>\.2025</g, `>${escapeXml(year)}<`);
  }
  xml = xml.replace(/>29\.08\.2025</g, `>${escapedDate}<`);
  return xml;
}
