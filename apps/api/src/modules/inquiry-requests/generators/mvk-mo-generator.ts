/**
 * Генератор справки МВК ЗСО (Московская область)
 * Шаблон: 4. МВК ЗСО.docx
 */
...
export interface MvkMoGeneratorParams {
  date: string;
  numberMiddle: string;
  year: string;
  objectName: string;
...
export interface MvkMoGeneratorResult {
  fileName: string;
  filePath: string;
  buffer: Buffer;
}
...
/**
 * Генерирует справку МВК ЗСО (МО)
 */
export async function generateMvkMoInquiry(
  templatePath: string,
  outputDir: string,
  params: MvkMoGeneratorParams,
): Promise<MvkMoGeneratorResult> {
  ...
  const safeObjectName = params.objectName
    ...
    .substring(0, 30)
    .trim();
  const fileName = `МВК_ЗСО_МО_${safeObjectName}_${Date.now()}.docx`;
