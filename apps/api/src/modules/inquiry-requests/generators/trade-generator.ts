/**
 * Генератор справки ДепТорговли Москвы
 * Шаблон: 5. ДепТорговли Москвы.docx
 */
...
export interface TradeGeneratorParams {
  date: string;
  numberMiddle: string;
  year: string;
  objectName: string;
...
export interface TradeGeneratorResult {
  fileName: string;
  filePath: string;
  buffer: Buffer;
}

export async function generateTradeInquiry(
  templatePath: string,
  outputDir: string,
  params: TradeGeneratorParams,
): Promise<TradeGeneratorResult> {
  ...
  const safeObjectName = params.objectName
    ...
    .substring(0, 30)
    .trim();
  const fileName = `ДепТорговли_${safeObjectName}_${Date.now()}.docx`;
