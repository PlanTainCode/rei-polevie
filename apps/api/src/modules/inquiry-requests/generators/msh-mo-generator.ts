/**
 * Генератор справки МСХ МО (Министерство сельского хозяйства МО - ветеринария)
 * Шаблон: 2. МСХ МО (ветеринария).docx
 */
...
export interface MshMoGeneratorParams {
  date: string;
  numberMiddle: string;
  year: string;
  objectName: string;
...
export interface MshMoGeneratorResult {
  fileName: string;
  filePath: string;
  buffer: Buffer;
}
...
/**
 * Генерирует справку МСХ МО (ветеринария)
 */
export async function generateMshMoInquiry(
  templatePath: string,
  outputDir: string,
  params: MshMoGeneratorParams,
): Promise<MshMoGeneratorResult> {
  ...
  const safeObjectName = params.objectName
    ...
    .substring(0, 30)
    .trim();
  const fileName = `МСХ_МО_${safeObjectName}_${Date.now()}.docx`;
