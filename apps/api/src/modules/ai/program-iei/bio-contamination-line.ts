...
export function removeCystsFromLine(text: string): string {
  let s = String(text || '');
  // Убираем упоминание цист в разных формулировках:
  //  - «, цисты простейших» (старый шаблон)
  //  - «, цист кишечных патогенных простейших» (актуальный шаблон, паразитологическая строка)
  // ВАЖНО: в JS \w НЕ включает кириллицу — используем явный диапазон [а-яё].
  s = s.replace(/,?\s*цист[а-яё]*\s+(?:кишечн[а-яё]*\s+)?(?:патоген[а-яё]*\s+)?просте(?:йш|и)[а-яё]*/gi, '');
  s = s.replace(/\s+,\s+,/g, ', ');
  s = s.replace(/\s+([,.])/g, '$1');
...
Нужно подготовить ФИНАЛЬНЫЙ текст санитарно-паразитологической строки таблицы 4.2 программы ИЭИ:
"Оценка санитарно-паразитологических показателей: жизнеспособные яйца и личинки гельминтов ..., цист кишечных патогенных простейших"

Правило:
- Если в поручении (orderText) есть "цисты простейших" или "цисты кишечных патогенных простейших" — оставь упоминание цист.
- Если в поручении цисты НЕ упоминаются — убери из строки фразу про цисты (оставив остальной список показателей, в т.ч. яйца и личинки гельминтов, без изменений).
