...
- Строку «Оценка содержания загрязняющих веществ...» (атмосферный воздух по справке ФГБУ «Центральное УГМС») оставляй ВСЕГДА — это не отбор проб, а оценка по справке.
- ППР (измерение плотности потока радона) оставляй ТОЛЬКО если в servicesFromOrder есть соответствующая услуга/строка про ППР-радон ИЛИ в поручении явно указано измерение ППР/плотности потока радона. Фраза «в соответствии с ППР» (проект производства работ) — НЕ основание оставлять строку ППР.
