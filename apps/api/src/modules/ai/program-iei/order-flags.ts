...
const heuristicFlagsFromText = (orderText: string): Omit<ProgramIeiOrderFlags, 'isCommunicationNetworksObject'> => {
  ...
  // ППР = плотность потока радона. Не путать с «проектом производства работ»
  // («в соответствии с ППР») — голый «ППР» без радонового/измерительного контекста не считаем.
  const textWithoutConstructionPpr = t
    .replace(/в\s+соответствии\s+с\s+ппр/gi, ' ')
    .replace(/проект\w*\s+производств\w*\s+работ/gi, ' ');
  const hasPPR =
    /(плотност\w*\s+поток\w*\s+радон|потоков\s+радона|измерен\w*[^\n]{0,60}\bппр\b|определен\w*[^\n]{0,60}\bппр\b|ппр\s*\([^)]*датчик|накопительн\w*[^\n]{0,80}\bппр\b|\bппр\b[^\n]{0,40}радон|радон[^\n]{0,40}\bппр\b)/i.test(
      textWithoutConstructionPpr,
    );
...
- hasPPR: есть ли измерение ППР как плотности потока радона (датчики/накопительные камеры). НЕ путать с «проектом производства работ» / фразой «в соответствии с ППР»
