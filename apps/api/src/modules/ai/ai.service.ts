...
export const TEMPLATE_SERVICES = [
  {
    ...
    unit: '1 га',
    keywords: ['радиометрическое обследование', 'радиометрическое', 'мэд', 'гамма', 'территория', 'га'],
  },
  {
    row: 17,
    num: 3,
  ...
  },
  {
    ...
    unit: '1000 кв.м',
    keywords: ['радиометрическое обследование здани', 'радиометрическое обследование помещени', 'мэд здани', 'мэд помещени'],
  },
  {
    row: 19,
    num: 5,
    ...
    unit: 'точка (замер)',
    keywords: ['эроа', 'объемная активность', 'оа радона', 'инспекционн'],
  },
  {
    row: 20,
    num: 6,
...
export class AiService {
  ...
  /**
   * Таблица 4.2: санитарно-паразитологическая строка (\"Оценка санитарно-паразитологических показателей...\").
   * Возвращает финальный текст строки с/без \"цист кишечных патогенных простейших\" по поручению.
   */
...
КРИТИЧЕСКИ ВАЖНО — РАДИОМЕТРИЧЕСКОЕ ОБСЛЕДОВАНИЕ:
- Если в поручении написано просто "Радиометрическое обследование" или "Радиометрия" (без уточнения "здания"/"помещения") и единица измерения в га — это строка 16 (Радиометрическое обследование ТЕРРИТОРИИ)
- Строка 18 (Радиометрическое обследование ЗДАНИЯ/помещений) — ТОЛЬКО если явно указаны здания, помещения, кв.м
- Не пропускай радиометрическое обследование! Это частая услуга в поручениях

КРИТИЧЕСКИ ВАЖНО — НЕ ПУТАЙ ЕРН и ЭРОА радона:
- ЕРН (естественные радионуклиды: цезий-137, калий-40, радий-226, торий-232) — это часть комплексного анализа ПОЧВ, входит в строку 20 (Санитарно-гигиеническое обследование почв - СанПиН). НЕ выделяй ЕРН как отдельную услугу!
- Строка 19 (Определение объемной активности / ЭРОА радона) — это ОТДЕЛЬНАЯ услуга для измерения радона ВНУТРИ ЗДАНИЙ/ПОМЕЩЕНИЙ. Она НЕ связана с ЕРН в почвах!
- Включай строку 19 ТОЛЬКО если в поручении ЯВНО упоминается: "ЭРОА", "объемная активность радона", "ОА радона", "инспекционный контроль радона" или измерение радона в здании/помещении
- Если в поручении упоминаются только ЕРН, радионуклиды в почве, цезий, радий, торий, калий-40 — это строка 20, НЕ строка 19

КРИТИЧЕСКИ ВАЖНО — НЕ ПУТАЙ ППР (радон) и «проект производства работ»:
- Строка 17 (ППР, плотность потока радона) — ТОЛЬКО если в поручении явно есть измерение/определение ППР, плотность потока радона, датчики/накопительные камеры ППР
- Фраза «в соответствии с ППР» или «проект производства работ» — это НЕ услуга строки 17, не включай её из‑за этого
...
3. **urbanPlanningActivity** - вид градостроительной деятельности.
   В ТЗ это может быть п.4 ИЛИ п.1.7 (нумерация разная) — ищи по названию «Вид градостроительной деятельности».
   СКОПИРУЙ значение 1:1 как в ТЗ (например «Реконструкция», «Строительство»).
...
- «Вид градостроительной деятельности» (п.4 или п.1.7) → urbanPlanningActivity (ОДНО значение 1:1 из ТЗ!)
...
- п.10.2 "Принадлежность к объектам транспортной инфраструктуры" → transportInfrastructure
  ВАЖНО: копируй ТОЧНО как написано в ТЗ! Если написано "Нет" — пиши "Нет", НЕ "Не принадлежит"!
- п.10.3 "Принадлежность к опасным производственным объектам" → hazardousProduction  
  ВАЖНО: копируй ТОЧНО как написано в ТЗ! Если написано "Нет" — пиши "Нет", НЕ "Не принадлежит"!
- п.10.4 "Пожарная и взрывопожарная опасность" → fireHazard
  ВАЖНО: копируй ТОЧНО как написано в ТЗ! Если написано "Нет данных" — пиши "Нет данных"!
...
ТИТУЛЬНАЯ СТРАНИЦА - извлеки ВСЕХ подписантов из шапки ТЗ:

В шапке ТЗ есть блоки "УТВЕРЖДАЮ" и/или "СОГЛАСОВАНО" с данными организаций.
Подписантов может быть 2, 3 или 4.

titleSignatories — массив ВСЕХ подписантов В ТОМ ПОРЯДКЕ, как они расположены на титуле ТЗ.
Для каждого подписанта укажи:
- header: "УТВЕРЖДАЮ" или "СОГЛАСОВАНО" (как написано перед его блоком). КОПИРУЙ ТОЧНО!
- label: роль — "Заказчик", "Технический заказчик", "Подрядчик", "Исполнитель", "Проектировщик" и т.д.
- position: должность (например "Генеральный директор", "Заместитель генерального директора по проектированию")
- organization: полное название организации (например "ООО «ВЗЛЕТ»", "АО «РЭИ-ЭКОАУДИТ»")
- name: ФИО в сокращённом формате (например "Е.В.Минаев", "М.А.Маренный"). ОБЯЗАТЕЛЬНОЕ ПОЛЕ — у каждого подписанта ВСЕГДА есть ФИО!
- basis: текст основания под ФИО, если есть (например "Доверенность №25/12/24-01 от 25.12.24г."). Если нет — пустая строка.

КРИТИЧЕСКИ ВАЖНО — каждый подписант это ВЕРТИКАЛЬНЫЙ БЛОК (столбец):
header → label → position → organization → name → подпись.
ВСЕ эти поля принадлежат ОДНОМУ подписанту! Не путай данные между столбцами!
ФИО стоит в том же столбце что и организация — они принадлежат одному подписанту.

При извлечении текста из docx таблица теряет структуру и текст из разных столбцов может идти подряд.
Чтобы не перепутать, используй ЛОГИЧЕСКУЮ привязку: ФИО → должность → организация (одна связка).

ПОРЯДОК чтения титула ТЗ: сверху вниз, слева направо.
Если подписанты расположены в ряд (2 рядом) — сначала левый, потом правый.

ПРИМЕР: если в шапке ТЗ видишь:
  УТВЕРЖДАЮ          СОГЛАСОВАНО
  Тех.заказчик       Проектировщик
  Директор           Зам.генерального директора
  ПАО МГТС           ООО «СТРОЙ»
  Д.П.Кондратов      А.В.Пономарев

То titleSignatories будет:
  [
    {"header":"УТВЕРЖДАЮ","label":"Технический заказчик","position":"Директор","organization":"ПАО МГТС","name":"Д.П.Кондратов","basis":""},
    {"header":"СОГЛАСОВАНО","label":"Проектировщик","position":"Зам.генерального директора","organization":"ООО «СТРОЙ»","name":"А.В.Пономарев","basis":"Доверенность №25/12/24-01 от 25.12.24г."}
  ]
Обрати внимание: Кондратов идёт с ПАО МГТС (один столбец), Пономарев с ООО «СТРОЙ» (другой столбец)!
Если под ФИО есть строка с доверенностью/приказом/основанием — копируй её в basis ДОСЛОВНО.

Также заполни старые поля для совместимости:
technicalCustomerName: название организации из блока "Технический заказчик" (например "ООО «ВЗЛЕТ»")
technicalCustomerDirectorPosition: должность из блока "Технический заказчик"
technicalCustomerDirectorName: ФИО из блока "Технический заказчик"
clientDirectorPosition: должность из блока "Заказчик"
clientDirectorName: ФИО из блока "Заказчик"
clientShortName: название без ООО/АО и кавычек (например "ГОРСВЯЗЬСТРОЙ")

ВАЖНО - ОПРЕДЕЛИ РОЛЬ АО «РЭИ-ЭКОАУДИТ»:
- В шапке ТЗ найди блок с организацией АО «РЭИ-ЭКОАУДИТ» (или просто РЭИ-ЭКОАУДИТ)
- contractorRole: "Подрядчик" или "Исполнитель" (по умолчанию "Подрядчик")
...
- backgroundConcentrationsRef: из п.22.5 ТЗ извлеки ТОЛЬКО номер и дату справки о фоновых концентрациях
  Пример в ТЗ: "Справка о фоновых концентрациях ... (выдана ФГБУ «Центральное УГМС» № 312/15/05/ Э-574 от 28.02.2022)"
  → Извлеки только: "№ 312/15/05/ Э-574 от 28.02.2022"
  Если нет такого пункта → пустая строка

- previousSurveyReport: из п.22.3 ТЗ СКОПИРУЙ ПОЛНОСТЬЮ текст про технический отчет по результатам ИЭИ (если такой пункт РЕАЛЬНО СУЩЕСТВУЕТ в ТЗ).
  ВАЖНО: НЕ КОПИРУЙ пример ниже — он дан только для понимания формата! Если в ТЗ нет п.22.3 или нет упоминания технического отчета — ОБЯЗАТЕЛЬНО верни пустую строку "".
  Формат (пример, НЕ копировать): "Технический отчет по результатам ... № XXX для объекта: «...». ООО «...», М.:20XX."

- reportCopiesText: из п.21 ТЗ "Требования к составу, форме и формату предоставления результатов инженерных изысканий"
  Нужны ТОЛЬКО количества экземпляров (бумажные / электронные) для п.7.2 Программы.
  НЕ копируй формулировки про форматы файлов (.pdf, .dwg, УКЭП и т.п.) — п.7.3 Программы не меняется, остаётся как в шаблоне.
  Если нет такого пункта → пустая строка
...
ФОРМАТ ОТВЕТА - СТРОГО JSON:
{
  ...
  "titleSignatories": [
    {"header": "СОГЛАСОВАНО", "label": "Технический заказчик", "position": "", "organization": "", "name": "", "basis": ""},
    {"header": "УТВЕРЖДАЮ", "label": "Подрядчик", "position": "Директор", "organization": "АО «РЭИ-ЭКОАУДИТ»", "name": "М.А.Маренный", "basis": ""},
    {"header": "СОГЛАСОВАНО", "label": "Заказчик", "position": "", "organization": "", "name": "", "basis": ""}
  ]
}
...
ИЗВЛЕЧЕНИЕ РАЙОНА МОСКВЫ (moscowDistrict):
- Если это г. Москва, ОБЯЗАТЕЛЬНО найди и укажи район!
- Район может быть указан:
  * В наименовании объекта: "р-н Фили-Давыдково", "район Хамовники"
  * В адресе: "район Хамовники", "Пресненский район"
  * Можно определить по улице: "ул. Арбат" → "Арбат", "Давыдковская ул." → "Фили-Давыдково"
- ПРИОРИТЕТ: сначала ищи явное указание района (р-н, район), потом определяй по улице
- Примеры:
  * "р-н Фили-Давыдково, Давыдковская ул." → moscowDistrict: "Фили-Давыдково"
  * "Ленинградский проспект, д.10" → moscowDistrict: "Аэропорт" или "Беговой"
- Формат: укажи ТОЛЬКО название района БЕЗ слова "район"/"р-н" (например: "Фили-Давыдково", а не "р-н Фили-Давыдково")
...
ФОРМАТ ОТВЕТА - СТРОГО JSON:
{
  ...
  private normalizeSection1Data(data: Partial<ProgramIeiSection1Data>): ProgramIeiSection1Data {
    ...
    return {
      ...
      reportCopiesText: String(data.reportCopiesText || '').trim(),
      contractorRole: this.normalizeContractorRole(data.contractorRole),
      titleSignatories: this.normalizeTitleSignatories(data.titleSignatories),
    };
  }

  private normalizeTitleSignatories(raw: any): TitleSignatory[] {
    if (!Array.isArray(raw) || raw.length === 0) return [];
    return raw
      .filter((s: any) => s && typeof s === 'object')
      .map((s: any) => ({
        header: String(s.header || 'СОГЛАСОВАНО').trim(),
        label: String(s.label || '').trim(),
        position: String(s.position || '').trim(),
        organization: String(s.organization || '').trim(),
  ...
  /**
   * Нормализует роль подрядчика: возвращает только "Подрядчик" или "Исполнитель"
   */
  private normalizeContractorRole(role: string | undefined): string {
    const normalized = String(role || '').trim().toLowerCase();
    if (normalized.includes('исполнител')) {
      return 'Исполнитель';
    }
    return 'Подрядчик';
  }
...
/**
 * Подписант с титульной страницы ТЗ.
 * Порядок в массиве = порядок на титуле (сверху вниз, слева направо).
 */
export interface TitleSignatory {
  header: string;        // "УТВЕРЖДАЮ" или "СОГЛАСОВАНО"
  label: string;         // "Заказчик", "Технический заказчик", "Подрядчик", "Исполнитель"
  position: string;      // "Генеральный директор", "Зам. директора"
  organization: string;  // "ООО «ВЗЛЕТ»", "АО «РЭИ-ЭКОАУДИТ»"
  name: string;          // "Е.В.Минаев"
  basis: string;         // "Доверенность №25/12/24-01 от 25.12.24г." (если есть под ФИО)
}
...
export interface ProgramIeiSection1Data {
  ...
  // Роль АО «РЭИ-ЭКОАУДИТ» в договоре: "Подрядчик" или "Исполнитель"
  // Определяется из шапки ТЗ (блок УТВЕРЖДАЮ со стороны РЭИ)
  contractorRole: string;
  
  // 2.1 Перечень исходных материалов и данных
  // п.22.5 ТЗ - номер и дата справки о фоновых концентрациях (например: "№ 312/15/05/ Э-574 от 28.02.2022")
  backgroundConcentrationsRef: string;
  // п.22.3 ТЗ - технический отчет по результатам ИЭИ (полный текст)
  previousSurveyReport: string;

  // 7.2 Количество экземпляров (ТЗ п.21 "Требования к составу, форме и формату предоставления результатов")
  reportCopiesText: string;

  // Подписанты с титульной страницы ТЗ (в порядке как на титуле: сверху вниз, слева направо)
  titleSignatories: TitleSignatory[];
}
