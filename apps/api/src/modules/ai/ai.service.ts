...
Твоя задача - найти и извлечь НАЗВАНИЕ заказчика из текста Технического Задания (ТЗ).

ВАЖНО:
- Ищи секцию с наименованием заказчика (обычно пункт 5.1)
- Возвращай ТОЛЬКО название организации (например: ООО «КАРАТ-91», АО «Строй-Инвест»)
- НЕ включай адрес, ОГРН, ИНН и другие реквизиты
- Если заказчик ИП — верни "ИП Фамилия Имя Отчество"

Отвечай ТОЛЬКО названием заказчика, без пояснений. Если не найден - верни пустую строку.`;

    try {
      const response = await this.chat([
        { role: 'system', content: systemPrompt },
        { role: 'user', content: `Найди название заказчика в тексте ТЗ:\n\n${tzText.substring(0, 5000)}\n\nНазвание заказчика:` },
      ]);
